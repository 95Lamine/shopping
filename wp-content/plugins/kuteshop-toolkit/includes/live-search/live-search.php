<?php
if ( !class_exists( 'Kuteshop_Live_Search' ) ) {
	class Kuteshop_Live_Search
	{
		...
		{
			if ( !isset( self::$instance ) && !( self::$instance instanceof Kuteshop_Live_Search ) ) {
				self::$instance = new Kuteshop_Live_Search;
			}
			self::includes();
			add_action( 'wp_enqueue_scripts', array( self::$instance, 'scripts' ), 1 );
			add_action( 'wp_ajax_kuteshop_live_search', array( self::$instance, 'get_results' ) );
			add_action( 'wp_ajax_nopriv_kuteshop_live_search', array( self::$instance, 'get_results' ) );
			add_shortcode( "kuteshop_live_search_form", array( self::$instance, 'live_search_form' ) );
		...
		{
			wp_enqueue_style( 'live-search', KUTESHOP_TOOLKIT_URL . 'includes/live-search/live-search.css' );
			wp_enqueue_script( 'kuteshop-live-search', KUTESHOP_TOOLKIT_URL . 'includes/live-search/live-search.js', array( 'jquery' ), '1.0', true );
			wp_localize_script( 'kuteshop-live-search', 'kuteshop_ajax_live_search', array(
					'ajaxurl'                             => admin_url( 'admin-ajax.php' ),
					'security'                            => wp_create_nonce( 'kuteshop_ajax_live_search' ),
					'view_all_text'                       => esc_html__( 'View All', 'kuteshop-toolkit' ),
					'product_matches_text'                => esc_html__( 'Product Matches', 'kuteshop-toolkit' ),
					'results_text'                        => esc_html__( 'Results', 'kuteshop-toolkit' ),
					'kuteshop_enable_live_search'         => isset( $this->options['enable_live_search'] ) ? $this->options['enable_live_search'] : 0,
					'kuteshop_live_search_min_characters' => isset( $this->options['min_characters'] ) ? $this->options['min_characters'] : 3,
				)
			);
		}
		...
		public function get_results()
		{
			...
			$GLOBALS['kuteshop_live_search_settings'] = $data;
			...
			if ( $products ) {
				foreach ( $products as $key => $product ) {
					...
					if ( $data['show_suggestion'] == 'on' && in_array( 'sku', $data['search_in'] ) ) {
						$products[$key]->kuteshop_sku = $product->get_sku();
					}
					if ( $product ) {
				...
				if ( $data['show_suggestion'] == 'on' ) {
					foreach ( $products as $product ) {
						...
						if ( in_array( 'sku', $data['search_in'] ) && !isset( $return_data['suggestion'] ) ) {
							// Convert HTML tag and shortcode to space.
							$content_search = preg_replace( '/\\[[^\\]]*\\]|<[^>]*>/', ' ', $product->kuteshop_sku );
							// Find keyword.
							$position_keyword = stripos( $content_search, $data['keyword'] );
							if ( $position_keyword !== false && $position_keyword + strlen( $data['keyword'] ) < strlen( $content_search ) ) {
								// Get suggestion of keyword in content.
			...
			wp_send_json( array( 'message' => esc_html__( 'No results.', 'kuteshop-toolkit' ) ) );
			wp_die();
		}
		...
		{
			global $wpdb, $kuteshop_live_search_settings;
			// Prepare search coverages.
			$columns = array();
			if ( in_array( 'title', $kuteshop_live_search_settings['search_in'] ) ) {
				$columns[] = ' ' . $wpdb->posts . '.post_title LIKE "%' . sanitize_text_field( $kuteshop_live_search_settings['keyword'] ) . '%" ';
			}
			if ( in_array( 'description', $kuteshop_live_search_settings['search_in'] ) ) {
				$columns[] = ' ' . $wpdb->posts . '.post_excerpt LIKE "%' . sanitize_text_field( $kuteshop_live_search_settings['keyword'] ) . '%" ';
			}
			if ( in_array( 'content', $kuteshop_live_search_settings['search_in'] ) ) {
				$columns[] = ' ' . $wpdb->posts . '.post_content LIKE "%' . sanitize_text_field( $kuteshop_live_search_settings['keyword'] ) . '%" ';
			}
			if ( in_array( 'sku', $kuteshop_live_search_settings['search_in'] ) ) {
				$columns[] = '( ' . $wpdb->postmeta . '.meta_key = "_sku" AND ' . $wpdb->postmeta . '.meta_value LIKE "%' . sanitize_text_field( $kuteshop_live_search_settings['keyword'] ) . '%" )';
			}
			if ( count( $columns ) ) {
		...
		{
			$default = array(
				'placeholder' => __( 'Search products', 'kuteshop-toolkit' ),
			);
			$atts    = shortcode_atts( $default, $atts );
			ob_start();
			?>
            <form method="get" action="<?php echo esc_url( home_url( '/' ) ) ?>"
                  class="block-search kuteshop-live-search-form">
				<?php if ( class_exists( 'WooCommerce' ) ): ?>
                    <input type="hidden" name="post_type" value="product"/>
				<?php endif; ?>
                <div class="search-box results-search">
                    <input autocomplete="off" type="text" class="searchfield txt-livesearch" name="s"
			...
			return apply_filters( 'kuteshop_output_live_search_form', $html, $atts );
		}
	}
}
$kuteshop_live_search = new Kuteshop_Live_Search();
$kuteshop_live_search::instance();
