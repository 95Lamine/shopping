		...
		function scripts()
		{
			...
			foreach ( $posts as $post ) {
				...
				if ( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'kuteshop_googlemap' ) ) {
					if ( $kuteshop_gmap_api_key ) {
						wp_enqueue_script( 'kuteshop-api-map', esc_url( '//maps.googleapis.com/maps/api/js?key=' . trim( $kuteshop_gmap_api_key ) ), array(), false, true );
					} else {
						wp_enqueue_script( 'kuteshop-api-sensor', esc_url( '//maps.google.com/maps/api/js?sensor=true' ), array(), false, true );
					}
				}
			}
